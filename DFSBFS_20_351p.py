...
단, 전체 선생님의 수는 5이하의 자연수,
전체 학생의 수는 30이하의 자연수이며 항상 빈 칸의 개수는 3개 이상으로 주어진다.
...
아이디어 :
수가 작아서 완전탐색으로 가능한 문제인 것 같다.
35C3 = 6545
선생님이 최대 5명이므로 50번

따라서 약 30만번이므로 완전 탐색 가능하다.


저자 아이디어 :
DFS BFS를 이용하여 모든 조합을 찾는 함수를 구현하는 것.
또는 조합 라이브러리를 이용하여 푸는 것


피드백 :
조합을 구현하는데 BFS DFS가 사용되는가 보다.
트리구조로 조합을 구할 수 있다.
꼭 BFS DFS를 이용해야하는 건 아니지만 DFS BFS를 이용해서
조합을 구할 수 있다.
...
commbinations(모든경우의 수 리스트 , 조합 개수)
