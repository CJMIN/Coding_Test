...
아이디어 :
모든 나라마다 BFS 혹은 DFS를 이용하여 이동을 시키면 될 것 같다.
연합이된 나라를 또 초기화되지 않도록 체크리스크를 만들자.
살짝 다이나믹 프로그래밍 같은 느낌이 난다.
이렇게 하면 칸수만큼 반복하여 초기화시킬 수 있다.
체크를 모두하면 1번 이동이다.

저자의 아이디어 :
모든 나라의 위치에서 DFS 혹은 BFS를 수행하여
인접한 나라의 인구수를 확인한 뒤에 가능하다면
국경선을 열고 인구이동 처리를 진행하면 된다고 한다.
저자는 완전 탐색으로 모든 나라를 전부 체크했다.



피드백 :
저자의 구현에서 인상이 깊었던 점은 1 <= nx < n 이다.
나는 아이디어에서 어떻게하면 종료시킬건지 구상하지 않았다.
끝까지 구상하는 습관을 들여야겠다.
