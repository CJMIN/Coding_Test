...
아이디어 :
1. 정렬시키기 (최대 10만log10만 = 약 170만 번)
2. 최소값 2개 더하기 (최대 10만 번)
3. 최소값 2개를 더한 값을 이진탐색을 이용하여 정렬시키기 (최대 17 번)
4. 2번과 3번 반복하기 (최대 10만 * 17 = 170만 번)
...
저자는 최소값 2개를 구하기 위하여 이진 탐색이 아니라 우선순위 큐를 이용하였다.
더 간단한 방법이다. 꼭 숙지해두자.
이 문제를 그리디의 한 유형이라고 한다.

피드백 :

힙 자료구조를 이용하여 구현한 우선순위 큐의 삽 시간과 삭제 시간은 logN이므로 N이 10만일 경우
약 17번의 연산이 필요하다.
